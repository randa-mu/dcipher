...
forge bind --bindings-path $ROOT_DIR/generated/src/randomness --skip-cargo-toml --module --select RandomnessSender --select TypesLib --select SignatureSender

cd $ROOT_DIR/blocklock-solidity
forge bind --bindings-path $ROOT_DIR/generated/src/blocklock --skip-cargo-toml --module --select BlocklockSender --select TypesLib --select DecryptionSender --select SignatureSender

cd $ROOT_DIR/onlyswaps-solidity
forge bind --bindings-path $ROOT_DIR/generated/src/onlyswaps --skip-cargo-toml --module --select Router --select IRouter --select ERC20FaucetToken --select SwapRequestParameters --select SwapRequestReceipt
